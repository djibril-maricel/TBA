...
class Quest:
    ...
    def check_item_objective(self, item_name, player=None):
        """
        Check if collecting a specific item completes an objective.
        
        Args:
            item_name (str): The name of the item collected.
            player: The player object (optional).
        ...
        >>> quest = Quest("Collect", "Collect a book", ["Récupérer book"])
        >>> quest.check_item_objective("book")
        ✅ Objectif accompli: Récupérer book
        <BLANKLINE>
        🏆 Quête terminée: Collect
        <BLANKLINE>
        True
        >>> quest.check_item_objective("lanterne")
        False
        """
        item_objectives = [
            f"Récupérer {item_name}",
            f"Collecter {item_name}",
            f"Ramasser {item_name}",
            f"Obtenir {item_name}"
        ]
    ...
    def check_character_objective(self, character_name, player=None):
        """
        Check if interact with a specific character completes an objective.
        ...
        >>> quest = Quest("Interact", "Interact with Michael", ["Parler à Michael"])
        >>> quest.check_character_objective("Mickael")
        ✅ Objectif accompli: Parler à Michael
        <BLANKLINE>
        🏆 Quête terminée: Interact
        <BLANKLINE>
        True
        >>> quest.check_character_objective("John")
        False
        """
        character_objectives = [
            f"Parler à {character_name}",
            f"Parler avec {character_name}",
            f"Interragir avec {character_name}",
            f"Communiquer avec {character_name}",
            f"Discuter avec {character_name}"
        ]

        for objective in character_objectives:
            if self.complete_objective(objective, player):
                return True
        return False
...
class QuestManager:
    ...
    def check_item_objectives(self, item_name):
        ...
        >>> manager = QuestManager()
        >>> quest = Quest("Collect item", "Collect axe", ["Récupérer hache"])
        >>> manager.add_quest(quest)
        >>> manager.activate_quest("Collect item") # doctest: +NORMALIZE_WHITESPACE
        <BLANKLINE>
        🗡️  Nouvelle quête activée: Collect item
        📝 Collect axe
        <BLANKLINE>
        True
        >>> manager.check_room_objectives("hache") # doctest: +NORMALIZE_WHITESPACE
        ✅ Objectif accompli: Récupérer hache
        <BLANKLINE>
        🏆 Quête terminée: Collect item
        <BLANKLINE>
        >>> len(manager.active_quests)
        0
        """
    ...
    def check_character_objectives(self, character_name):
        """
        Check all active quests for character-related objectives.
        
        Args:
            character_name (str): The name of the characetr.
            
        Examples:
        
        >>> manager = QuestManager()
        >>> quest = Quest("Interract", "Interract with John", ["Parler à John"])
        >>> manager.add_quest(quest)
        >>> manager.activate_quest("Interract") # doctest: +NORMALIZE_WHITESPACE
        <BLANKLINE>
        🗡️  Nouvelle quête activée: Interract
        📝 Parler à John
        <BLANKLINE>
        True
        >>> manager.check_character_objectives("John") # doctest: +NORMALIZE_WHITESPACE
        ✅ Objectif accompli: Parler à John
        <BLANKLINE>
        🏆 Quête terminée: Interract
        <BLANKLINE>
        >>> len(manager.active_quests)
        0
        """
        for quest in self.active_quests[:]:  # Use slice to avoid modification during iteration
            quest.check_character_objective(character_name, self.player)
            if quest.is_completed:
                self.active_quests.remove(quest)
